...
This repository contains a small web store application that is used to develop and validate AI agents, skills and plugins in the software engineering life cycle. When present in a repo all engineering team members benefit from the agent expert guidance and are able to use and benefit from AI support equally - no matter the experience level.

Today it is thought that most engineers operate at the Yegge phase 2 level - Coders in the IDE (https://steve-yegge.medium.com/welcome-to-gas-town-4f25ee16dd04). The messaging related to use and benefits of AI in engineering teams does not seem to be  conclusive and may even be somehwat contradictory. It is not clear who benefits most, the senior engineer or junior. Senior already generate significant amounts of code that they may not never full review themselves, leaving team member peer reviewers with significant challenges in terms of the scope of code to be reviewed in a pull request. Team throughput is increasing, but not equally so.

There is a question as what would a mature engineering team look like in a future state AI Native/Hybrid engineering team where roles have changes and coder engineers turn into builders for example and large amounts of code is generated by LLMs in IDEs and CLI interfaces turning daily activities into AI chats rather than centered around line-by-line coding. And do we support everyone YOLO'ing, or do we provide a form of guidance where we harness the LLM with guidance that we develop and maintain to the benefit of the engineering organization and continued quality of deliverables?

In this expreriment, we develop and experiment with AI agents support the team in the following ways in the project:
- A product owner creates requirements that is reveiewed on and eloborated by an AI agent with direct Jira integration.
- Senior engineers use the eloborate requirements to generaate an technical implementation plan.
- Quality engineers benefit from Gherkin spec developed and maintained of requirements, design and implemented code.
