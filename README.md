...
This repository contains a small web store application that is used to develop and validate AI agents, skills and plugins in the software engineering life cycle. When present in a repo all engineering team members benefit from AI agent expert guidance and are able to use and benefit from the AI support equally - irrespective of experience level.

Today, it is thought that most engineers operate at the Yegge phase 2 level - Coders in the IDE (https://steve-yegge.medium.com/welcome-to-gas-town-4f25ee16dd04). The messaging related to use and benefits of AI in engineering teams does not seem to be  conclusive and may even be somehwat contradictory. It is not agreed on as to who benefits most, the senior engineer or junior. Seniors already generate significant amounts of code that they may never fully inspect themselves, leaving team member peer reviewers with significant challenges in terms of the scope of changes to be reviewed in pull requests. Team throughput is increasing, but not necessarily equally so.

To solo YOLO or not: There is a question as what would a mature engineering team would look like in a future state AI Native/Hybrid engineering team where roles have changed, coders turn into builder and for large amounts of code are generated by LLMs in IDE/CLI interfaces turning their daily activities into AI chats rather than line-by-line coding. And do we support everyone developng their own practices, or do we consider providing forms of guidance where we harness the AI support with guidance that the team develops and maintains to the benefit of the organization and continued quality of deliverables?

In this project, we develop and experiment with AI agents intended to support the team in the following use cases:
- A product owner creates requirements that is reveiewed on and eloborated on by an AI agents with direct Jira integration.
- Senior engineers use the eloborated requirements to generaate a impact analysis and technical implementation plans.
- Quality engineers benefit from realtime Gherkin spec driven test automation (functional and accessibility).
- DevSecOps is represented by LLM best practices knowledge through design and code reviews and external tools e.g. SNYK.
- Architects or Lead Engineers may request the LLM to review a set of requirements, architectural guidance, and explore code base to come up with a set of technical epics intended to evolve the application into a best practices, and create prioritized user stories!

This project and repo currently is developed in Cursor with GPT 5.3 Codex as of 03/14/2026.

Some of the findings and open questions as to date:
- We typically go from Jira ticket to code complete in 30 under minutes, auto code reviewed and with full test automation! 
- The size of the ticket (user story or smaller epic) do not seem to impact to matter all that much as coding happens blazingly fast. 
- The project agent definitions enable full tracebility of the process, from requirements elaboration through done-done! The noore we exercise and enhance the agents, the more reliable the process guidance and do's and don't become. AI likes to code and haas no problem skipping e.g requirements clarification technical analysis if not guided properly. And in doing so we loose traceability, which may or may not fit orgabizational ways of working and compliance requirements. With our current approach, the agents report their work and timing back a requirements file in this repo that is based on the original requirement (manually created or via Jira) and attaches a completed history of the engineering efforts to the source Jira ticket. The hope is that this can be used to complement the review process and it provides a significant degree tracebility at the issue level. 
- What is the impact of a much larger repo on th effectiveness of this agents approach to the life cycle?
- What kind of buy-in would we get in the engineering team and to what extend may this be considered limiting an engineers creativity and freedowm? 
-- Note: It is possible to by-pass documented requirements and provide a requirement directly in chat. Code generation proceeds. However, with the current agent definitions, it starts following the guidance of next in line agents, including AGENTS.md, AGENT-CYPRESS.md, etc.! And the final worklow check would detect the absence of documented requirements, and has successfully offered to retro fit the project with Jira tickets - attempting to recover them from the chat history and context. 
- Agents, skills and plugins definitions continue to standardizing across vendors. To what extent are these agents usable as is in Cursor vs. CLaude Code or Codex for example. 
